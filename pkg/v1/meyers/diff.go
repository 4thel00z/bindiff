...
import (
	"bytes"
	diff "github.com/4thel00z/bindiff/pkg/v1"
)
...
func ComputeEdits(uri diff.URI, before, after []byte) ([]diff.TextEdit, error) {
	ops := operations(splitLines(before), splitLines(after))
	edits := make([]diff.TextEdit, 0, len(ops))
	for _, op := range ops {
		s := diff.New(uri, diff.NewPoint(op.I1+1, 1, 0), diff.NewPoint(op.I2+1, 1, 0))
		switch op.Kind {
		case diff.Delete:
		...
		case diff.Insert:
			...
			if content := bytes.Join(op.Content, []byte{}); len(content) != 0 {
...
func operations(a, b [][]byte) []*operation {
	if len(a) == 0 && len(b) == 0 {
		return nil
	}
...
func shortestEditSequence(a, b [][]byte) ([][]int, int) {
	M, N := len(a), len(b)
	V := make([]int, 2*(N+M)+1)
	offset := N + M
	...
	for d := 0; d <= N+M; d++ {
		...
		for k := -d; k <= d; k += 2 {
			...
			for x < M && y < N && bytes.Compare(a[x], b[y]) == 0 {
				x++
				y++
			}
